# ============================================================
# FIX + FITUR: Halaman publik /testimoni 500 error (Undefined
# variable $totalTestimoni) + menyelesaikan fitur admin
# "Manajemen Testimoni" yang kepotong di tengah jalan.
#
# ROOT CAUSE:
# resources\views\pages\frontend\testimoni.blade.php (halaman
# PUBLIK /testimoni) ketiban isi komponen Livewire untuk PANEL
# ADMIN (referensi route admin.testimonials.create/edit, butuh
# $totalTestimoni dari method with()). Padahal route publik
# /testimoni cuma passing $testimonials biasa -> 500 error.
#
# File asli halaman publik masih aman di:
#   testimoni.blade.php.bak-20260830
#
# APA YANG DILAKUKAN SCRIPT INI:
# 1. Backup semua file yang akan disentuh (routes/web.php,
#    layouts/admin-panel.blade.php, frontend/testimoni.blade.php).
# 2. Simpan isi Livewire admin yang nyasar tadi ke lokasi yang
#    BENAR: resources\views\pages\admin\testimoni.blade.php
# 3. Restore resources\views\pages\frontend\testimoni.blade.php
#    dari .bak-20260830 (halaman publik normal lagi).
# 4. Buat file baru resources\views\pages\admin\testimoni-form.blade.php
#    (form Tambah/Edit Testimoni, pola sama seperti kategori-form,
#    termasuk cropper foto rasio 1:1/lingkaran).
# 5. Tambah 3 route baru di routes\web.php (grup admin, setelah
#    route Interaksi): admin.testimonials / .create / .edit
# 6. Tambah menu "Testimoni" di sidebar admin (layouts/admin-panel.blade.php),
#    diletakkan sebelum Pengaturan (dekat Interaksi, sesuai tema fitur).
#
# TIDAK ADA file lain yang disentuh. Tidak ada npm run build yang
# diperlukan (murni PHP/Blade, bukan CSS/JS baru).
#
# Cara pakai:
# 1. Jalankan dari folder project (C:\xampp\htdocs\karyaIdeEdi)
# 2. .\apply-fix-testimoni-misplaced-and-admin-page.ps1
# ============================================================
...
Write-Host " Fix: /testimoni 500 error + fitur admin Testimoni" -ForegroundColor Cyan
Write-Host "==============================================" -ForegroundColor Cyan
Write-Host ""
...
$frontendTestimoni = "resources\views\pages\frontend\testimoni.blade.php"
$frontendTestimoniBak = "resources\views\pages\frontend\testimoni.blade.php.bak-20260830"
$adminTestimoni = "resources\views\pages\admin\testimoni.blade.php"
$adminTestimoniForm = "resources\views\pages\admin\testimoni-form.blade.php"
$routesFile = "routes\web.php"
$layoutFile = "resources\views\layouts\admin-panel.blade.php"

foreach ($f in @($frontendTestimoni, $frontendTestimoniBak, $routesFile, $layoutFile)) {
    if (-not (Test-Path $f)) {
...
if (Test-Path $adminTestimoni) {
    Write-Host "[ERROR] File sudah ada: $adminTestimoni" -ForegroundColor Red
    Write-Host "Kemungkinan script ini sudah pernah dijalankan. Cek manual ya, ge." -ForegroundColor Yellow
    exit 1
}

# ---------- [1/6] Backup ----------
Copy-Item $frontendTestimoni "$frontendTestimoni.bak-broken-admin-content-$stamp" -Force
Copy-Item $routesFile "$routesFile.bak-before-testimoni-admin-$stamp" -Force
Copy-Item $layoutFile "$layoutFile.bak-before-testimoni-admin-$stamp" -Force
Write-Host "[1/6] Backup dibuat untuk 3 file yang akan diubah." -ForegroundColor Green

$utf8Bom = New-Object System.Text.UTF8Encoding $true

# ---------- [2/6] Pindahkan isi admin yang nyasar ke lokasi yang benar ----------
$misplacedAdminContent = Get-Content $frontendTestimoni -Raw -Encoding UTF8
[System.IO.File]::WriteAllText((Join-Path (Get-Location) $adminTestimoni), $misplacedAdminContent, $utf8Bom)
Write-Host "[2/6] Isi admin dipindah ke: $adminTestimoni" -ForegroundColor Green

# ---------- [3/6] Restore halaman publik dari backup asli ----------
$originalPublicContent = Get-Content $frontendTestimoniBak -Raw -Encoding UTF8
[System.IO.File]::WriteAllText((Resolve-Path $frontendTestimoni), $originalPublicContent, $utf8Bom)
Write-Host "[3/6] Halaman publik /testimoni di-restore ke versi asli." -ForegroundColor Green

# ---------- [4/6] Buat file form Tambah/Edit Testimoni ----------
$formContent = @'
<?php
...
'@
[System.IO.File]::WriteAllText((Join-Path (Get-Location) $adminTestimoniForm), $formContent, $utf8Bom)
Write-Host "[4/6] File form dibuat: $adminTestimoniForm" -ForegroundColor Green

# ---------- [5/6] Tambah route admin.testimonials* ----------
$routesContent = Get-Content $routesFile -Raw -Encoding UTF8

$routeAnchor = "    Route::livewire('/interaksi', 'pages::admin.interaksi')->name('interaksi');"
$routeNew = @"
...
if (-not $routesContent.Contains($routeAnchor)) {
    Write-Host "[ERROR] Baris acuan route Interaksi tidak ditemukan di $routesFile." -ForegroundColor Red
    Write-Host "Kemungkinan file sudah pernah diubah manual. Cek manual ya, ge." -ForegroundColor Yellow
    Write-Host "Backup tetap aman di: $routesFile.bak-before-testimoni-admin-$stamp" -ForegroundColor Yellow
    exit 1
}

$routesContent = $routesContent.Replace($routeAnchor, $routeNew.TrimEnd("`r", "`n"))
[System.IO.File]::WriteAllText((Resolve-Path $routesFile), $routesContent, $utf8Bom)
Write-Host "[5/6] Route admin.testimonials / .create / .edit ditambahkan." -ForegroundColor Green

# ---------- [6/6] Tambah menu Testimoni di sidebar admin ----------
...
if (-not $layoutContent.Contains($navAnchor)) {
    Write-Host "[ERROR] Blok `$otherNav tidak ditemukan persis di $layoutFile." -ForegroundColor Red
    Write-Host "Kemungkinan file sudah pernah diubah manual. Cek manual ya, ge." -ForegroundColor Yellow
    Write-Host "Backup tetap aman di: $layoutFile.bak-before-testimoni-admin-$stamp" -ForegroundColor Yellow
    Write-Host "" -ForegroundColor Yellow
    Write-Host "Route & halaman testimoni admin TETAP sudah aktif (langkah 1-5 berhasil)." -ForegroundColor Yellow
    Write-Host "Cuma menu sidebar-nya yang perlu ditambah manual, atau akses langsung ke /admin/testimoni." -ForegroundColor Yellow
    exit 1
}

$layoutContent = $layoutContent.Replace($navAnchor, $navNew)
[System.IO.File]::WriteAllText((Resolve-Path $layoutFile), $layoutContent, $utf8Bom)
Write-Host "[6/6] Menu 'Testimoni' ditambahkan di sidebar admin." -ForegroundColor Green

Write-Host ""
Write-Host "==============================================" -ForegroundColor Cyan
Write-Host " SELESAI!" -ForegroundColor Cyan
Write-Host " - /testimoni (publik) sudah normal lagi." -ForegroundColor Cyan
Write-Host " - /admin/testimoni (Manajemen Testimoni) sudah aktif," -ForegroundColor Cyan
Write-Host "   lengkap dengan Tambah/Edit + upload foto (crop 1:1)." -ForegroundColor Cyan
Write-Host " - Cek php artisan route:list --name=testimonials" -ForegroundColor Cyan
Write-Host "   untuk pastikan 3 route baru sudah terdaftar." -ForegroundColor Cyan
Write-Host " Hard refresh browser (Ctrl+Shift+R) untuk keduanya." -ForegroundColor Cyan
Write-Host "==============================================" -ForegroundColor Cyan
