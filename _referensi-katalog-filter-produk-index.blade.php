...
<body class="min-h-screen bg-[#FEEDD8] font-sans antialiased text-[#2A211B]">
    @include('partials.frontend.shop-header')

    {{-- ================= SHOP HERO / BREADCRUMB ================= --}}
    <section class="bg-[#F6F9F6]">
        <div class="mx-auto flex min-h-[175px] max-w-[1180px] flex-col items-center justify-center px-5 py-12 text-center sm:px-7 lg:px-8">
                    ...
                    <aside class="lg:pt-1">
                        ...
                        <div class="mt-10">
                            ...
                            <div class="mt-2 flex items-center justify-between text-[10px] text-[#75685B]">
                                <span id="min-price-label">Rp{{ number_format((float) (request('min_price', $catalogMinPrice)), 0, ',', '.') }}</span>
                                <span>-</span>
                                <span id="max-price-label">Rp{{ number_format((float) (request('max_price', $catalogMaxPrice)), 0, ',', '.') }}</span>
                            </div>
                            ...
                            <div class="relative mt-5 h-5">
                                ...
                                <input
                                    id="min-price-range"
                                    type="range"
                                    min="{{ (int) floor($catalogMinPrice) }}"
                                    max="{{ (int) ceil($catalogMaxPrice) }}"
                                    value="{{ (int) request('min_price', $catalogMinPrice) }}"
                                    step="1000"
                                ...
                                <input
                                    ...
                                    min="{{ (int) floor($catalogMinPrice) }}"
                                    max="{{ (int) ceil($catalogMaxPrice) }}"
                                    value="{{ (int) request('max_price', $catalogMaxPrice) }}"
                                    step="1000"
                                    aria-label="Harga maksimum"
                                    class="price-range pointer-events-none absolute inset-x-0 top-0 h-5 w-full appearance-none bg-transparent"
                                >
                                <input id="min-price-input" type="hidden" name="min_price" value="{{ (int) request('min_price', $catalogMinPrice) }}">
                                <input id="max-price-input" type="hidden" name="max_price" value="{{ (int) request('max_price', $catalogMaxPrice) }}">
                            </div>
                        </div>
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-2 py-5">
                            ...
                            @if (request()->filled('min_price') || request()->filled('max_price'))
                                <span class="inline-flex items-center gap-2 rounded-sm bg-[#FBE4C5] px-2.5 py-1.5 text-[9px] font-medium text-[#8C6A45]">
                                    Price: Rp{{ number_format((float) request('min_price', $catalogMinPrice), 0, ',', '.') }} - Rp{{ number_format((float) request('max_price', $catalogMaxPrice), 0, ',', '.') }}
                                </span>
                            @endif
    ...
    @include('partials.frontend.shop-footer')
