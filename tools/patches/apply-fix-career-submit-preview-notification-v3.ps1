...
$backupDir = ".backup-career-submit-notif-preview-v3-$stamp"
...
Write-Host " Career - Resume Fix Submit + Preview + Notifikasi (v3)" -ForegroundColor Yellow
Write-Host "====================================================================" -ForegroundColor Yellow
...
if ($formText -notmatch "careerPreviewSelectedFile\(this,\s*['""]cv['""]\)") {
    $cvPattern = '(<input\b(?=[^>]*name\s*=\s*["'']cv_file["''])[^>]*)(>)'
    $cvRegex = [System.Text.RegularExpressions.Regex]::new(
        $cvPattern,
        [System.Text.RegularExpressions.RegexOptions]::IgnoreCase
    )

    $newFormText = $cvRegex.Replace(
        $formText,
        '$1 onchange="careerPreviewSelectedFile(this, ''cv'')"$2',
        1
    )

    if ($newFormText -eq $formText -or $newFormText -notmatch "careerPreviewSelectedFile\(this,\s*['""]cv['""]\)") {
...
Write-Host "  - Bug overload Regex PowerShell dari v2 diperbaiki." -ForegroundColor White
