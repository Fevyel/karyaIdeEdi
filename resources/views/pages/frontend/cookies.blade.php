...
<head>
    ...
    <title>Cookies — {{ \App\Models\Setting::current()->site_name }}</title>
...
<body class="min-h-screen bg-white font-sans text-[#2A211B] antialiased">
    ...
    @php
        ...
        // Penyimpanan lokal (favorit & keranjang) memakai localStorage browser,
        // bukan cookie — tapi tetap dijelaskan di sini karena secara fungsi
        // serupa (data tersimpan di perangkat pembeli) dan sering ditanyakan.
        $cookieTypes = [
            [
                'icon' => 'fa-gear',
                'name' => 'Esensial',
                'status' => 'always',
                'desc' => 'Diperlukan agar situs berfungsi dengan baik — misalnya menjaga sesi login admin tetap aktif dan keamanan formulir. Tidak dapat dinonaktifkan karena situs tidak akan berjalan normal tanpanya.',
            ],
            [
                'icon' => 'fa-heart',
                'name' => 'Preferensi Lokal',
                'status' => 'local',
                'desc' => 'Favorit dan Keranjang belanja Anda disimpan di localStorage browser perangkat ini (bukan cookie, dan bukan di server kami) agar tetap tersimpan saat Anda kembali berkunjung — tanpa perlu membuat akun.',
            ],
            [
                'icon' => 'fa-link',
                'name' => 'Tautan Lacak Pesanan',
                'status' => 'local',
                'desc' => 'Setelah pesanan dibuat, token pelacakan pesanan Anda didaftarkan pada perangkat ini supaya halaman Lacak Pesanan bisa langsung menampilkan riwayat pesanan Anda tanpa harus login.',
            ],
            [
                'icon' => 'fa-chart-line',
                'name' => 'Analitik Pihak Ketiga',
                'status' => 'none',
                'desc' => 'Kami tidak memasang Google Analytics, Meta Pixel, atau alat pelacak iklan pihak ketiga mana pun di situs ini.',
            ],
            [
                'icon' => 'fa-bullhorn',
                'name' => 'Iklan &amp; Retargeting',
                'status' => 'none',
                'desc' => 'Kami tidak membagikan data kunjungan Anda kepada jaringan iklan, dan tidak menampilkan iklan pihak ketiga di situs ini.',
            ],
        ];

        $cookieStatusMap = [
            'always' => ['label' => 'Selalu Aktif', 'color' => 'bg-admin-accent text-white'],
            'local'  => ['label' => 'Tersimpan di Perangkat Anda', 'color' => 'bg-admin-cream text-admin-accent'],
            'none'   => ['label' => 'Tidak Digunakan', 'color' => 'bg-[#F2F2F2] text-[#8A8A8A]'],
        ];
    @endphp
    ...
    <section class="relative overflow-hidden bg-[#1A1A1A]">
        ...
        <div class="relative mx-auto max-w-4xl px-6 py-16 text-center sm:px-8 lg:py-20">
            ...
            <p class="mx-auto mt-5 max-w-2xl text-sm leading-relaxed text-white/60 sm:text-base">
                Kami menjaga situs ini seringan dan sesederhana mungkin. Berikut rincian
                lengkap dan jujur soal data apa saja yang tersimpan di perangkat Anda saat
                berkunjung ke {{ $legalSetting->site_name }}.
            </p>
        ...
        <div class="mx-auto max-w-5xl px-6 py-10 sm:px-8">
            <div class="flex flex-col items-start gap-4 rounded-2xl border border-admin-accent/20 bg-white p-6 sm:flex-row sm:items-center sm:p-7">
                ...
                <p class="text-sm leading-relaxed text-[#4A423B] sm:text-[15px]">
                    <span class="font-semibold text-[#1A1A1A]">Singkatnya:</span>
                    kami tidak memasang cookie pelacak iklan atau analitik pihak ketiga.
                    Yang kami simpan di perangkat Anda hanya hal-hal yang membuat pengalaman
                    belanja lebih nyaman — seperti daftar favorit dan keranjang Anda sendiri.
                </p>
            </div>
        </div>
    </section>
    ...
    <section class="bg-white">
        <div class="mx-auto max-w-5xl px-6 py-14 sm:px-8 lg:py-20">
            ...
            <p class="mt-2 max-w-2xl text-sm leading-relaxed text-[#6B6E76]">
                Kategori esensial &amp; preferensi lokal tidak dapat kami hindari karena
                menjadi dasar fungsi belanja di situs ini. Sisanya, secara sadar tidak
                kami gunakan.
            </p>

            <div class="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2">
                @foreach ($cookieTypes as $cookie)
                    @php $statusInfo = $cookieStatusMap[$cookie['status']]; @endphp
            ...
            <div class="mt-12 rounded-2xl border border-[#1A1A1A]/10 bg-admin-cream/30 p-6 sm:p-8">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="font-display text-lg font-semibold text-[#1A1A1A]">
                        Mengatur Penyimpanan di Browser Anda
                    </h3>
                </div>
                <p class="mt-3 text-sm leading-relaxed text-[#4A423B] sm:text-[15px]">
                    Karena favorit dan keranjang tersimpan secara lokal di browser Anda,
                    Anda bisa menghapusnya kapan saja melalui pengaturan "Clear browsing
                    data" / "Hapus data situs" pada browser yang Anda gunakan. Menghapus
                    data ini hanya akan mengosongkan favorit &amp; keranjang di perangkat
                    tersebut — tidak memengaruhi pesanan yang sudah dikonfirmasi.
                </p>
            </div>
            ...
            <div class="mt-8 rounded-2xl bg-[#1A1A1A] p-8 text-center sm:p-10">
                ...
                <p class="mx-auto mt-2 max-w-md text-sm leading-relaxed text-white/60">
                    Kami senang menjelaskan lebih lanjut soal bagaimana situs ini
                    bekerja.
                </p>
