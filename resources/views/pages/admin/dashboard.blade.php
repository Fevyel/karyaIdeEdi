...
new #[Layout('layouts::admin-panel')] #[Title('Dashboard')] class extends Component
{
    ...
    public function with(): array
    {
        ...
        $categoryTotals = Transaction::query()
            ...
            ->selectRaw('categories.name as category, SUM(transactions.total) as total')
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <span class="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold
                                    {{ $item->approval_status === 'approved' ? 'bg-green-50 text-green-600' : ($item->approval_status === 'rejected' ? 'bg-red-50 text-red-500' : 'bg-amber-50 text-amber-600') }}">
                                    {{ match ($item->approval_status) { 'approved' => 'Disetujui', 'rejected' => 'Ditolak', default => 'Menunggu' } }}
                                </span>
                            </div>
