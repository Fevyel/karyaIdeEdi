...
new #[Layout('layouts::admin-panel')] #[Title('History Pesanan')] class extends Component
{
    use WithPagination;

    public string $search = '';

    /** 'all', 'completed', atau 'cancelled' — subset dari Transaction::FINAL_STATUSES. */
    ...
    /**
     * Sama seperti Pesanan: begitu History dibuka, pesanan final yang
     * belum sempat "dibaca" admin (mis. langsung completed/cancelled
     * sebelum halaman Pesanan aktif sempat dibuka) ikut ditandai sudah
     * dibaca di sini juga — supaya badge notifikasi tidak pernah
     * nyangkut menghitung baris yang sudah tidak mungkin lagi muncul
     * di halaman Pesanan (baris itu sudah difilter keluar dari sana).
     */
    ...
    {
        $valid = array_merge(['all'], Transaction::FINAL_STATUSES);
    ...
    public function with(): array
    {
        $query = Transaction::query()
            ...
            // Kebalikan dari halaman Pesanan: HANYA status final.
        ...
        return [
            ...
            'totalHistory' => Transaction::query()->whereIn('status', Transaction::FINAL_STATUSES)->count(),
            'detailItem' => $this->showDetail && $this->detailId
                ? Transaction::query()->with('product:id,nama')->whereIn('status', Transaction::FINAL_STATUSES)->find($this->detailId)
                : null,
        ];
    }
};
...
<div class="space-y-6">
        ...
        <div class="relative overflow-hidden rounded-2xl border border-admin-border bg-admin-surface px-6 py-20 text-center shadow-sm">
            ...
            <div class="relative">
                ...
                <p class="mt-5 font-display text-base font-semibold text-admin-ink">
                    {{ $search !== '' || $statusFilter !== 'all' ? 'Tidak ada history yang cocok' : 'Belum ada history pesanan' }}
                </p>
                <p class="mx-auto mt-1.5 max-w-sm text-xs leading-relaxed text-admin-ink-soft">
                    ...
                    @else
                        Pesanan yang sudah ditandai selesai atau dibatalkan akan muncul di sini.
                    @endif
                </p>
            </div>
        </div>
                ...
                <table class="w-full min-w-230 text-left text-sm">
                    ...
                    <tbody class="divide-y divide-admin-border">
                        @foreach ($transactions as $transaction)
                            @php
                                $statusStyle = $transaction->status === 'completed'
                                    ? ['dot' => 'bg-admin-success', 'pill' => 'bg-admin-success/10 text-admin-success', 'label' => 'Selesai']
                                    : ['dot' => 'bg-admin-danger', 'pill' => 'bg-admin-danger/10 text-admin-danger', 'label' => 'Dibatalkan'];
                            @endphp
                            <tr class="transition-colors duration-200 hover:bg-admin-canvas">
                                ...
                                <td class="px-3 py-3 text-admin-ink-soft">
                                    {{ $transaction->product?->nama ?? '&mdash;' }}
                                </td>
                                <td class="px-3 py-3 text-admin-ink">
                                ...
                                <td class="px-5 py-3">
                                    <div class="flex items-center justify-end gap-1.5" wire:key="aksi-history-{{ $transaction->id }}">
                                        <button
                                            type="button"
    ...
    {{-- ================= MODAL: DETAIL HISTORY (read-only, tanpa aksi) ================= --}}
    @if ($showDetail && $detailItem)
        @php
            $detailStatusStyle = $detailItem->status === 'completed'
                ? ['dot' => 'bg-admin-success', 'pill' => 'bg-admin-success/10 text-admin-success', 'label' => 'Selesai']
                : ['dot' => 'bg-admin-danger', 'pill' => 'bg-admin-danger/10 text-admin-danger', 'label' => 'Dibatalkan'];
            ...
            $waMessage = $trackingUrl
                ? "Halo {$detailItem->customer_name}, pesanan Anda di Karya Ide Edi dengan kode {$detailItem->order_code} ".($detailItem->status === 'completed' ? 'telah selesai' : 'dibatalkan').".\n\nAnda dapat melihat detail pesanan melalui link berikut:\n{$trackingUrl}\n\nTerima kasih,\nKarya Ide Edi"
                : null;
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="admin-scroll relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-admin-surface shadow-2xl">
                <div class="sticky top-0 z-10 flex items-center justify-between border-b border-admin-border bg-admin-surface px-6 py-4">
                    <h3 class="font-display text-lg font-semibold text-admin-ink">Detail History Pesanan</h3>
                ...
                <div class="space-y-5 px-6 py-5">
                    ...
                    <div class="grid grid-cols-2 gap-4 rounded-xl border border-admin-border bg-admin-canvas p-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm font-semibold text-admin-ink">{{ $detailItem->product?->nama ?? '&mdash;' }}</p>
                        </div>
                        <div>
                    ...
                    @if ($detailItem->catatan)
                        <div>
                            <p class="text-[11px] uppercase tracking-wide text-admin-ink-soft">Catatan</p>
                            <p class="mt-1 rounded-lg bg-admin-canvas px-3 py-2 text-sm text-admin-ink">{{ $detailItem->catatan }}</p>
                        </div>
                    @endif

                    <div class="rounded-xl border border-admin-border bg-admin-canvas p-4">
                        <p class="flex items-start gap-1.5 text-xs leading-relaxed text-admin-ink-soft">
                            <i class="fa-solid fa-circle-info mt-0.5 text-admin-accent"></i>
                            @if ($detailItem->status === 'completed')
                                Pesanan ini telah selesai diproses dan sudah keluar dari antrean aktif. Data ini bersifat riwayat (read-only).
                            @else
                                Pesanan ini telah dibatalkan dan sudah keluar dari antrean aktif. Data ini bersifat riwayat (read-only).
                            @endif
                        </p>
                    </div>

                    @if ($detailItem->tracking_token)
                        <div>
                            <p class="text-[11px] font-semibold uppercase tracking-wide text-admin-ink-soft">Tracking Pesanan</p>
